...
export class MeetupStateService {
  private readonly MEETUP_EVENT_URL =
    'https://api.meetup.com/<%= meetupName %>/events?&sign=true&photo-host=public&status=past,upcoming';
  ...
  private eventsLoadingSubject$: BehaviorSubject<boolean> = new BehaviorSubject<
    boolean
  >(false);
  ...
  get upcomingEvents$(): Observable<Event[]> {
    return this.eventsSubject$
      .asObservable()
      .pipe(map(results => results.filter(result => result.status !== 'past')));
  }
